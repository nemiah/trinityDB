...
*                                                                              *
*******************************************************************************/
function FPDF($orientation='P',$unit='mm',$format='A4',$copy = false)
{
	//Some checks
	$this->_dochecks();
...
function AddPage($orientation='',$resetFakePageCounter = false)
{
	...
	if($resetFakePageCounter) $this->fakePage = 0;
	//Start new page
...
function Output($name='',$dest='')
{
	...
	switch($dest)
	{
		...
		case 'F':
			...
			$f=fopen($name,'wb');
			if(!$f)
				$this->Error('Das PDF kann nicht zwischengespeichert werden!'."<br /><br />Bitte machen Sie das Verzeichnis<br />system/IECache<br />sowie dessen Unterverzeichnisse durch den Webserver beschreibbar:<br /><br /><code>chmod -R 777 system/IECache</code>".(ini_get("safe_mode") ? "<br /><br /><span style=\"color:red;\">Auf diesem Webserver wurde der Safe Mode aktiviert.<br />Bitte beachten Sie die Hinweise in der <a href=\"../system/info.php\">info.php-Datei</a></span>" : ""));
...
function _putpages()
{
	...
	if(count($this->replaceKeyword) > 0){
		#echo "hier";
		for($n=1;$n<=$nb;$n++)for($i=0;$i<count($this->replaceKeyword);$i++) $this->pages[$n]=str_replace($this->replaceKeyword[$i],$this->replaceWith[$i],$this->pages[$n]);
	}
